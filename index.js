...
app.get('/mark-found', async (req, res) => {
  ...
  try {
    ...
    // Get metafields for the page
    const metafields = await axios.get(`https://${SHOPIFY_SHOP}.myshopify.com/admin/api/2023-10/pages/${page.id}/metafields.json`, {
      headers: {
        'X-Shopify-Access-Token': SHOPIFY_TOKEN
      }
    });

    const lostStatusField = metafields.data.metafields.find(mf => mf.namespace === 'custom' && mf.key === 'lost_status');
